...
const spins_service_1 = require("../../src/modules/spins/spins.service");
describe("SpinsService", () => {
    it("applies winning spin", () => {
        const store = new store_1.DataStore();
        const service = new spins_service_1.SpinsService(store, () => 0.99);
    ...
    it("applies losing spin", () => {
        const store = new store_1.DataStore();
        const service = new spins_service_1.SpinsService(store, () => 0.0);
        const result = service.spin("p1", 100);
    ...
    it("accepts bet exactly equal to player balance (boundary: exact balance)", () => {
        const store = new store_1.DataStore();
        const service = new spins_service_1.SpinsService(store, () => 0.99); // win
